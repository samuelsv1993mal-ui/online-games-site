...
const I18N = {
  ru: {
    appTitle:'SMQ Games', appSubtitle:'Играй онлайн, с другом, против компьютера или наблюдай за комнатой.', loginGoogle:'Войти через Google', continueGuest:'Продолжить как гость', googleHint:'Google сохраняет фото, цвет профиля, настройки и историю игр.', installApp:'Скачать как приложение', installUnavailable:'Установка появится после открытия сайта в Chrome/браузере и обновления страницы.', installReady:'Можно установить SMQ Games как приложение.', appInstalled:'SMQ Games установлено.', updateApp:'Обновить приложение', updateChecking:'Проверяем обновление...', updateReady:'Обновляем приложение...', offlineWifi:'Офлайн по Wi‑Fi', offlineWifiTitle:'Локальная Wi‑Fi комната', offlineWifiDesc:'Когда нет интернета, запусти этот проект на телефоне в Termux. Друзья подключаются к твоей точке доступа и открывают локальный адрес.', showLocalAddress:'Показать адрес сервера', localAddress:'Адрес для друзей', copyAddress:'Скопировать адрес', localNoAddress:'Локальный IP пока не найден. Включи точку доступа/Wi‑Fi и перезапусти сервер.', localSteps:'1) Включи точку доступа. 2) Запусти сервер в Termux. 3) Друзья подключаются к Wi‑Fi и открывают адрес ниже.',
...
const games = [
  ...
  { id:'memory', emoji:'🧠', max:2 }, { id:'twentyone', emoji:'21', max:2 }, { id:'reaction', emoji:'⚡', max:2 }, { id:'checkers', emoji:'⚫', max:2 }, { id:'chess', emoji:'♞', max:2 }, { id:'domino', emoji:'🁫', max:2 },
...
function displayPlayerName(p) {
  if (!p) return 'Player';
  if (p.isBot) {
    const diff = state.room?.difficulty || state.room?.requestedDifficulty || 'normal';
    const diffText = t(diff) || t('normal');
    return `SMQ Bot · ${diffText}`;
  }
...
function renderPlayerStrip(room) {
  ...
  strip.innerHTML = activePlayers(room).map(p => `<div class="player-chip" style="--pc:${p.color || '#8b5cf6'}">${playerAvatar(p, 'md')}<b>${escapeHtml(displayPlayerName(p))}${p.isBot ? ' 🤖' : ''}</b><em>${room.game === 'teamquiz' ? `${t('team')} ${p.team + 1} · ` : ''}${room.scores[p.index] || 0}</em></div>`).join('');
}
...
function dominoTileHtml(tile, extra = '') {
  if (!tile) return '';
  const a = Number(tile.left ?? tile.a ?? 0);
  const b = Number(tile.right ?? tile.b ?? 0);
  const dbl = a === b ? ' double' : '';
  return `<span class="domino-tile ${extra}${dbl}"><b>${a}</b><i></i><b>${b}</b></span>`;
}
function dominoCanPlace(tile, line, side) {
  if (!tile) return false;
  if (!line || !line.length) return true;
  const left = Number(line[0].left);
  const right = Number(line[line.length - 1].right);
  if (side === 'left') return tile.a === left || tile.b === left;
  if (side === 'right') return tile.a === right || tile.b === right;
  return false;
}
function renderDomino(board, room) {
  const s = room.state || {};
  const idx = myIndex();
  const hand = idx >= 0 ? (s.hands?.[idx] || []) : [];
  const line = s.line || [];
  const canPlay = canAct(true) && idx >= 0;
  const legalCount = hand.filter(tile => dominoCanPlace(tile, line, 'left') || dominoCanPlace(tile, line, 'right')).length;
  const opponent = room.players?.[idx === 0 ? 1 : 0];
  const opponentHand = idx >= 0 ? (s.hands?.[idx === 0 ? 1 : 0]?.length ?? 0) : 0;
  const last = s.lastMove ? `<div class="checkers-status">${escapeHtml(s.lastMove)}</div>` : `<div class="checkers-status">${t('dominoHint')}</div>`;
  const tableTiles = line.length ? line.map(tile => dominoTileHtml(tile)).join('') : `<div class="domino-empty">${t('dominoBoard')}</div>`;
  const handHtml = hand.length ? hand.map(tile => {
    const canLeft = canPlay && dominoCanPlace(tile, line, 'left');
    const canRight = canPlay && dominoCanPlace(tile, line, 'right');
    const placeOne = canPlay && !line.length;
    return `<div class="domino-hand-item">${dominoTileHtml(tile, canLeft || canRight || placeOne ? ' playable' : '')}<div class="domino-actions">${placeOne ? `<button class="chip-btn" data-domino-place="${tile.id}" data-side="right">${t('selectMove')}</button>` : `${canLeft ? `<button class="chip-btn" data-domino-place="${tile.id}" data-side="left">${t('leftSide')}</button>` : ''}${canRight ? `<button class="chip-btn" data-domino-place="${tile.id}" data-side="right">${t('rightSide')}</button>` : ''}`}</div></div>`;
  }).join('') : `<div class="domino-empty">${t('dominoBlocked')}</div>`;
  const drawDisabled = !canPlay || legalCount > 0;
  board.innerHTML = `<div class="domino-wrap"><div class="domino-top"><div class="total-card"><small>${t('dominoBoard')}</small><div class="domino-line">${tableTiles}</div></div><div class="domino-info"><span>${t('boneyard')}: <b>${s.boneyardCount ?? 0}</b></span><span>${opponent ? playerLabel({...opponent, index: idx === 0 ? 1 : 0}, 'xs') : t('opponentTurn')}: <b>${opponentHand}</b> ${t('tiles')}</span></div></div>${last}<div class="domino-hand"><h3>${t('myTiles')}</h3><div class="domino-hand-grid">${handHtml}</div><button class="btn secondary full" id="dominoDrawBtn" ${drawDisabled ? 'disabled' : ''}>${(s.boneyardCount || 0) > 0 ? t('drawTile') : t('passMove')}</button></div></div>`;
  board.querySelectorAll('[data-domino-place]').forEach(btn => btn.onclick = () => sendAction({ type:'place', tileId: btn.dataset.dominoPlace, side: btn.dataset.side }));
  $('dominoDrawBtn') && ($('dominoDrawBtn').onclick = () => sendAction({ type:'draw' }));
}
...
async function showLocalWifiInfo() {
  const box = $('localWifiBox');
  if (!box) return;
  box.classList.remove('hidden');
  box.innerHTML = `<div class="local-loading">${t('updateChecking')}</div>`;
  try {
    const res = await fetch('/api/local-info', { cache: 'no-store' });
    const data = await res.json();
    const urls = Array.isArray(data.localUrls) ? data.localUrls : [];
    if (!urls.length) {
      box.innerHTML = `<p class="hint">${t('localNoAddress')}</p><p class="hint">${t('localSteps')}</p>`;
      return;
    }
    box.innerHTML = `<p class="hint">${t('localSteps')}</p><div class="local-address-list">${urls.map((item, idx) => `
      <div class="local-address-card">
        <small>${escapeHtml(item.interface || 'Wi‑Fi')}</small>
        <strong>${escapeHtml(item.url)}</strong>
        <button class="btn secondary small" type="button" data-copy-local="${idx}">${t('copyAddress')}</button>
      </div>`).join('')}</div>`;
    box.querySelectorAll('[data-copy-local]').forEach(btn => {
      btn.onclick = () => {
        const item = urls[Number(btn.dataset.copyLocal)];
        if (item?.url) copy(item.url);
      };
    });
  } catch (error) {
    box.innerHTML = `<p class="hint">${t('localNoAddress')}</p><p class="hint">${escapeHtml(String(error.message || error))}</p>`;
  }
}
...
function bindEvents() {
  ...
  $('localWifiBtn') && ($('localWifiBtn').onclick = showLocalWifiInfo);
