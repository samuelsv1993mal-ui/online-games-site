...
const games = [
  ...
  { id:'nim', emoji:'🪵', max:2 }, { id:'code', emoji:'🔐', max:1 }, { id:'millionaire', emoji:'💰', max:10 }, { id:'teamquiz', emoji:'🏁', max:10 }, { id:'mathrace', emoji:'🧮', max:10 }
];
...
function renderTurnBanner(room) {
  ...
  const idx = myIndex();
  if (['millionaire','teamquiz','mathrace'].includes(room.game)) { banner.textContent = room.state.answers?.[String(idx)] ? t('answered') : t('yourTurn'); return; }
...
function renderGame(room) {
  ...
  const map = { rps:renderRps, ttt:renderTtt, dice:renderDice, connect4:renderConnect4, memory:renderMemory, twentyone:renderTwentyOne, reaction:renderReaction, checkers:renderCheckers, nim:renderNim, code:renderCode, millionaire:renderMillionaire, teamquiz:renderMillionaire, mathrace:renderMathRace };
...
function renderMillionaire(board, room) {
  ...
  board.innerHTML = `<div class="quiz-wrap"><div class="quiz-score">${isTeam ? room.teamScores.map((x,i)=>`${t('team')} ${i+1}: ${s.teamPoints?.[i] ?? 0}`).join(' · ') : activePlayers(room).map(p=>`${p.name}: ${s.points?.[p.index] ?? 0}`).join(' · ')}</div><div class="question-card"><small>${t('question')} ${s.qIndex + 1}/${s.questions.length}</small><h2>${q.q}</h2></div><div class="answer-grid">${q.a.map((a,i) => `<button class="answer-btn ${s.showAnswer ? (i === s.showAnswer.correct ? 'correct' : (s.answers?.[String(idx)] === i ? 'wrong' : '')) : ''}" data-answer="${i}" ${!canAct(false) || answered || s.showAnswer ? 'disabled' : ''}>${String.fromCharCode(65+i)}. ${a}</button>`).join('')}</div>${answered && !s.showAnswer ? `<p class="hint-line">${t('answered')}</p>` : ''}</div>`;
