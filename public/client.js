...
function renderWhoAmI(board, room) {
  ...
  const lang = state.lang === 'es' ? 'es' : 'ru';
  const qText = s.question ? (s.question[lang] || s.question.ru || s.question.es || '') : '';
  ...
  if (s.phase === 'spinning') {
    ...
    body = `<div class="whoami-stage question-stage">
      ...
      <div class="whoami-question-card">
        ...
        <h2>${qText}</h2>
        <p>${t('whoamiPassHint')} ${original !== holder ? `${original} получает новый вопрос, если ответ засчитан.` : ''}</p>
      </div>
  ...
  board.innerHTML = `<div class="whoami-wrap"><div class="whoami-score-grid">${scoreCards}</div>${hostControls}<div class="whoami-event">${s.lastEvent || ''}</div>${body}</div>`;
